...
svit = Service('拒绝cbi', bundle='pcr查询', help_= '简化cbi指令，目前有：\n预约, 状态', enable_on_default=False)
...
@svit.on_prefix(('预约'))
async def yuyue(bot, ev: CQEvent):
    try:    
        boss = int(ev['message'][0]['data']['text']) - 1
        qq = ev.user_id
        group_id =str(ev.group_id)
        msg = add_reservation(group_id, boss, qq)
        await bot.send(ev, msg)
    except:
        await bot.send(ev, '请输入正确的格式,例如：预约5')
