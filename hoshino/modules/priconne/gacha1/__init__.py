...
[切换卡池] 更换模拟卡池
'''.strip()
sv = Service('pcr-gacha', help_=sv_help, bundle='pcr娱乐')
...
gacha_10_aliases = ('来十连','抽十连', '十连', '十连！', '十连抽', '来个十连', '来发十连', '来次十连', '抽个十连', '抽发十连', '抽次十连', '十连扭蛋', '扭蛋十连',
...
gacha_1_aliases = ('来单抽','单抽', '单抽！', '来发单抽', '来个单抽', '来次单抽', '扭蛋单抽', '单抽扭蛋',
                   '單抽', '單抽！', '來發單抽', '來個單抽', '來次單抽', '轉蛋單抽', '單抽轉蛋')
gacha_300_aliases = ('一井','抽一井', '来一井', '来发井', '抽发井', '天井扭蛋', '扭蛋天井', '天井轉蛋', '轉蛋天井')
...
async def gacha_10(bot, ev: CQEvent):
    ...
    if hiishi == 19:
        ...
        await bot.send(ev,f'\n海豹退散！\n{res}', at_sender=True)
        await silence(ev.ctx,60)
    else:
        await bot.send(ev,f'\n运气挺好的！\n{res}', at_sender=True)
    #await silence(ev.ctx, silence_time)
...
async def gacha_300(bot, ev: CQEvent):
    ...
    if not up == 0 :
        judge_luck = up * 200 + s3 * 100 + 300 - int(result['first_up_pos'])
    else:
        judge_luck = up * 200 + s3 * 100

    if judge_luck <= 400:
        msg.append("太惨了，咱们还是退款删游吧...")
    elif up == 0 and judge_luck >= 800 :
        msg.append("up呢？我的up在哪儿呢？")
    elif up == 0:
        msg.append("据说天井的概率只有12.16%")
    elif judge_luck <= 700 :
        msg.append("学医救不了非洲人")
    elif judge_luck <= 1000 :
        msg.append("今日好像不宜抽卡")
    elif judge_luck <= 1200:
        msg.append("期望之内，亚洲水平")
    elif judge_luck <= 1400:
        msg.append("你，你是要准备偷渡欧洲？")
        await silence(ev, 120)
    elif judge_luck <= 1700:
        msg.append("那边的欧洲人，我来给你加状态")
        await silence(ev, 300)
    else:
        msg.append("说，你是托，说不出来就算了")
        await silence(ev, 600)
